    ...
    <>
      ...
      <section className="relative z-10 px-3 sm:px-6">
        <div className="mx-auto max-w-4xl border border-stone-200 bg-white shadow-lg">
          ...
          <p className="mb-6">There are two kinds of functions; <strong>semantic</strong> and <strong>pragmatic</strong>; both are important to a healthy codebase.</p>
          ...
          <p className="mb-6">Models should be self describing as well. While <code className="rounded bg-stone-100 px-1.5 py-0.5 text-sm">{"{"} id: {'"'}123{'"'} {"}"}</code> and <code className="rounded bg-stone-100 px-1.5 py-0.5 text-sm">{"{"} id: {'"'}123{'"'} {"}"}</code> may look the same without context, in context one might be a <code className="rounded bg-stone-100 px-1.5 py-0.5 text-sm">DocumentReference</code> and the other a <code className="rounded bg-stone-100 px-1.5 py-0.5 text-sm">MessagePointer</code>. To solve for this, wherever possible, brand types. In Rust this looks like making your IDs <code className="rounded bg-stone-100 px-1.5 py-0.5 text-sm">DocumentId(UUID)</code> instead of <code className="rounded bg-stone-100 px-1.5 py-0.5 text-sm">UUID</code>. This will stop mixups from functions receiving incorrect data that{"'"}s in the right shape.</p>
          <p className="mt-8 mb-3 font-bold">Where Things Break</p>
          <p className="mb-8">Breaks commonly happen when a semantic function morphs into a pragmatic function for ease, and then other places in the codebase that rely on it end up doing things they didn{"'"}t intend. To solve this, be explicit when creating a function by naming it instead of by what it does, but by where it{"'"}s used. The nature of their names should make it clear to other programmers in their names that their behavior is not tightly defined and should not be relied on for the internals to do an exact task, and make debugging regressions from them easier.</p>
          </div>
        </div>
      </section>
